...
size_t mark_band_border(uint8_t* f, size_t x, size_t y, size_t w, uint8_t mask){
	uint32_t i = y * w;
	i += x;
...
size_t mark_band_centre(uint8_t* f, size_t x, size_t y, size_t w) {
	uint32_t i = y * w;
	i +=  x;
	if (f[i] != INSIDE) {
		// The pixel must be inside the inpaint area for it to be a candidate for marking as BAND.
...
size_t mark_band(uint8_t* f, size_t w, size_t h)
{
	...
	for (size_t y = 1; y < h-1; y++) {
		for (size_t x = 1; x < w-1; x++) {
			found_pixels += mark_band_centre(f, x, y, w); 		 // Centre plane
		}
		found_pixels += mark_band_border(f, 0, y, w, LEFT_COLUMN);      // left column
		found_pixels += mark_band_border(f, w-1, y, w, RIGHT_COLUMN);   // right column
	}

	for (size_t x = 1; x < w-1; x++) {
		found_pixels += mark_band_border(f, x, h-1, w, BOTTOM_ROW);     // bottom row
		found_pixels += mark_band_border(f, x, 0, w, TOP_ROW);          // Top row
	}

	found_pixels += mark_band_border(f, 0, 0, w, TOP_LEFT_CORNER);      // Top left
	found_pixels += mark_band_border(f, 0, h-1, w, BOTTOM_LEFT_CORNER); // Bottom left
	found_pixels += mark_band_border(f, w-1, 0, w, TOP_RIGHT_CORNER);   // Top right
	found_pixels += mark_band_border(f, w-1, h-1, w, BOTTOM_RIGHT_CORNER); // Bottom right

	return found_pixels;
}

void inpaint_pixel_border(uint8_t* f, uint8_t* r, uint8_t* g, uint8_t* b, size_t x, size_t y, size_t w, uint8_t mask){
	uint32_t i = y * w;
	i += x;
	if (f[i] == BAND) {
		float r_total_weight = 0; // Sum of weights of KNOWN neighbours.
		float g_total_weight = 0; // Sum of weights of KNOWN neighbours.
		float b_total_weight = 0; // Sum of weights of KNOWN neighbours.
		float r_value = 0; // Sum of weighted values of KNOWN neighbours.
		float g_value = 0; // Sum of weighted values of KNOWN neighbours.
		float b_value = 0; // Sum of weighted values of KNOWN neighbours.
		// If any of the 8 neighbouring pixels is KNOWN, the current pixel is marked as BAND.
		// Bound checks are necessary in case the current pixel is on the border of the image (since then the neighbour is outside of the image).
		...
		element *= w;
		element += x - 1;
		if ((mask & TL) && f[element] == KNOWN) {
			r_value += 0.707 * r[element];
			g_value += 0.707 * g[element];
			b_value += 0.707 * b[element];
			r_total_weight += 0.707;
			g_total_weight += 0.707;
			b_total_weight += 0.707;
		}

		// Top-center
		element++;
		if ((mask & TC) && f[element] == KNOWN) {
			r_value += r[element];
			g_value += g[element];
			b_value += b[element];
			r_total_weight++;
		...
		if (r_total_weight == 0 || g_total_weight == 0 || b_total_weight == 0) {
		...
		r[i] = (uint8_t)(r_value / r_total_weight);
		g[i] = (uint8_t)(g_value / g_total_weight);
		b[i] = (uint8_t)(b_value / b_total_weight);
	}
}

void inpaint_pixel_centre(uint8_t* f, uint8_t* r, uint8_t* g, uint8_t* b, size_t x, size_t y, size_t w){
	uint32_t i = y * w;
	i += x;
	if (f[i] == BAND) {
		float r_total_weight = 0; // Sum of weights of KNOWN neighbours.
...
void inpaint_pixel(uint8_t* r, uint8_t* g, uint8_t* b, uint8_t* f, size_t w, size_t h)
{
	size_t wShift = w-1;
	size_t hShift = h-1;
	inpaint_pixel_border(f, r, g, b, 0, hShift, w, BOTTOM_LEFT_CORNER); // Bottom left
	for (size_t x = 1; x < w-1; x++) {
		inpaint_pixel_border(f, r, g, b, x, hShift, w, BOTTOM_ROW);     // bottom row
	}
	inpaint_pixel_border(f, r, g, b, wShift, hShift, w, BOTTOM_RIGHT_CORNER); // Bottom right
	
	for (size_t y = 1; y < hShift; y++) {
		inpaint_pixel_border(f, r, g, b, 0, y, w, LEFT_COLUMN);      // left column
		for (size_t x = 1; x < wShift; x++) {
			inpaint_pixel_centre(f, r, g, b, x, y, w); 		 // Centre plane
		}
		inpaint_pixel_border(f, r, g, b, wShift, y, w, RIGHT_COLUMN);   // right column
	}

	inpaint_pixel_border(f, r, g, b, 0, 0, w, TOP_LEFT_CORNER);      // Top left
	for (size_t x = 1; x < wShift; x++) {
		inpaint_pixel_border(f, r, g, b, x, 0, w, TOP_ROW);          // Top row
	}
	inpaint_pixel_border(f, r, g, b, wShift, 0, w, TOP_RIGHT_CORNER);   // Top right
}
