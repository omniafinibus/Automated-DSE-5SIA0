...
#define C_KNOWN KNOWN
#define C_INSIDE INSIDE
#define C_BAND BAND
...
size_t mark_band(flag_t* f, size_t w, size_t h)
{
	...
	for (size_t y = 0; y < h; y++)
	{
		for (size_t x = 0; x < w; x++)
		{
			...
			{
				unsigned int element =  y - 1;
				element *= w;
				element +=  x - 1;
				if (f[element] == C_KNOWN)
				{
					f[i] = C_BAND;
					found_pixels++;
					continue;
...
{
	unsigned int total_weight = 0; // Sum of weights of KNOWN neighbours.
	unsigned int value = 0; // Sum of weighted values of KNOWN neighbours.
	unsigned int wShift = w - 1;
	unsigned int hShift = h - 1;
	unsigned int yNeg = y - 1;
	unsigned int yPos = y + 1;
...
bool inpaint(uint8_t* r, uint8_t* g, uint8_t* b, bool* m, size_t w, size_t h)
{
	...
	{
		f[index] = m[index] ? C_INSIDE : C_KNOWN;
	}
